...
final familyHistoryRepositoryProvider = Provider<FamilyHistoryRepository>(
  (ref) => FamilyHistoryRepository(firestore: ref.watch(_firestoreProvider)),
);

final familyHistoryListProvider =
    NotifierProvider<FamilyHistoryListNotifier, FamilyHistoryListUiState>(
      FamilyHistoryListNotifier.new,
    );

@immutable
class FamilyHistoryListUiState {
  const FamilyHistoryListUiState({
    this.items = const [],
  ...
  static const initial = FamilyHistoryListUiState();

  FamilyHistoryListUiState copyWith({
    ...
    return FamilyHistoryListUiState(
      items: items == _sentinel
          ? this.items
          : items as List<FamilyHistoryEntry>,
...
class FamilyHistoryListNotifier extends Notifier<FamilyHistoryListUiState> {
  @override
  FamilyHistoryListUiState build() => FamilyHistoryListUiState.initial;
  ...
  Future<void> refresh({bool preserveItems = true}) async {
    state = state.copyWith(
      ...
      clearCursor: true,
      hasMore: true,
      items: preserveItems ? state.items : const <FamilyHistoryEntry>[],
    );
  ...
  Future<void> deleteEntry(FamilyHistoryEntry entry) async {
    final uid = _uid;
    if (uid == null) return;
    ...
    try {
      ...
      if (!res.success) {
        state = state.copyWith(
          clearDeleting: true,
          error: res.errorMessage ?? 'Could not delete family history entry.',
        );
        return;
      }
      state = state.copyWith(
  ...
  Future<void> _fetchFirstPage({bool preserveItems = false}) async {
    ...
    if (uid == null) {
      state = state.copyWith(
        ...
        error: 'Not signed in',
        items: const <FamilyHistoryEntry>[],
        hasMore: false,
        clearCursor: true,
      );
      return;
