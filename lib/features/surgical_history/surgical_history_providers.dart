...
final surgicalHistoryRepositoryProvider = Provider<SurgicalHistoryRepository>((
  ref,
) {
  return SurgicalHistoryRepository(firestore: ref.watch(_firestoreProvider));
});

final surgicalHistoryListProvider =
    NotifierProvider<SurgicalHistoryListNotifier, SurgicalHistoryListUiState>(
      SurgicalHistoryListNotifier.new,
    );

@immutable
class SurgicalHistoryListUiState {
  const SurgicalHistoryListUiState({
    this.items = const [],
  ...
  static const initial = SurgicalHistoryListUiState();

  SurgicalHistoryListUiState copyWith({
    ...
    return SurgicalHistoryListUiState(
      items: items == _sentinel
          ? this.items
          : items as List<SurgicalHistoryEntry>,
...
class SurgicalHistoryListNotifier extends Notifier<SurgicalHistoryListUiState> {
  @override
  SurgicalHistoryListUiState build() => SurgicalHistoryListUiState.initial;
  ...
  Future<void> refresh({bool preserveItems = true}) async {
    state = state.copyWith(
      ...
      clearCursor: true,
      hasMore: true,
      items: preserveItems ? state.items : const <SurgicalHistoryEntry>[],
    );
  ...
  Future<void> deleteEntry(SurgicalHistoryEntry entry) async {
    final uid = _uid;
    if (uid == null) return;
    ...
    try {
      ...
      if (!res.success) {
        state = state.copyWith(
          clearDeleting: true,
          error: res.errorMessage ?? 'Could not delete surgical history entry.',
        );
        return;
      }
      state = state.copyWith(
  ...
  Future<void> _fetchFirstPage({bool preserveItems = false}) async {
    ...
    if (uid == null) {
      state = state.copyWith(
        ...
        error: 'Not signed in',
        items: const <SurgicalHistoryEntry>[],
        hasMore: false,
        clearCursor: true,
      );
      return;
