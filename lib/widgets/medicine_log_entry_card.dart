...
class MedicineLogEntryCard extends StatelessWidget {
  const MedicineLogEntryCard({
    super.key,
    required this.entry,
    required this.onTap,
    this.onDelete,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${loc.formatMediumDate(entry.loggedAt)} • ${loc.formatTimeOfDay(TimeOfDay.fromDateTime(entry.loggedAt))}',
                          ...
                          if (entry.isTaken) ...[
                            const SizedBox(height: AppSpacing.xs),
                            Text(
                              '${entry.units} ${entry.units == 1 ? 'unit' : 'units'} from inventory',
                              style: theme.textTheme.bodySmall?.copyWith(
                                color: scheme.onSurfaceVariant,
                              ),
                            ),
  ...
  Color _statusColor() {
    switch (entry.status) {
      case MealStatuses.taken:
        return AppColors.premium;
      case MealStatuses.missed:
